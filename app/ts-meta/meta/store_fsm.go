...
func (fsm *storeFSM) executeCmd(cmd proto2.Command) interface{} {
	...
	case proto2.Command_UpdateSchemaKeySetCommand:
		return fsm.applyUpdateSchemaKeySetCommand(&cmd)
...
func (fsm *storeFSM) applyUpdateSchemaKeySetCommand(cmd *proto2.Command) interface{} {
	ext, _ := proto.GetExtension(cmd, proto2.E_UpdateSchemaKeySetCommand_Command)
	v, ok := ext.(*proto2.UpdateSchemaKeySetCommand)
	if !ok {
		panic(fmt.Errorf("%s is not a UpdateSchemaKeySetCommand", ext))
	}
	return fsm.data.UpdateSchemaKeySet(v.GetDatabase(), v.GetRpName(), v.GetMeasurement(), v.GetIds(), v.GetFieldToCreate(), time.Unix(0, v.GetTimestamp()))
}
