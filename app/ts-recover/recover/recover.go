...
func (r *RecoverOptions) recoverMeta(backupPath string, dbs []string) error {
	...
	if len(dbs) > 0 {
		...
		u := fmt.Sprintf("%s://%s/recoverMeta", protocol, r.Host)
		urlValues := url.Values{}
		urlValues.Add(backup.DataBases, strings.Join(dbs, ","))
		urlValues.Add(backup.MetaData, string(buf))

		res, err := r.sendRequestToMeta(u, http.MethodPost, urlValues)
		if err != nil {
			return err
		}
...
func (r *RecoverOptions) sendRequestToMeta(u string, typ string, urlValues url.Values) (*http.Response, error) {
	Url, err := url.Parse(u)
	if err != nil {
		return nil, err
	}
...
func resolveResponse(resp *http.Response) (*http.Response, error) {
	if resp.StatusCode == http.StatusOK {
		return resp, nil
	}
	b, err := io.ReadAll(resp.Body)
	if err != nil {
		return resp, fmt.Errorf("recover meta request error,read response body faild,%s", err.Error())
	}

	return resp, fmt.Errorf("recover meta error,%s", string(b))
}
	...
	for _, fd := range fds {
		...
		if fd.IsDir() {
			if fd.Name() == config.IndexFileDirectory && (!isInc || incBackup) {
				prefix := r.FullBackupDataPath
				if incBackup {
					prefix = r.IncBackupDataPath
				}
				outPath := strings.Replace(srcfp, filepath.Join(prefix, backup.DataBackupDir), "", -1)
...
func (r *RecoverOptions) genMap() error {
	...
	metaInfo, err := r.getMetaInfo(backupMetaPath)
	if err != nil {
		return err
	}
...
func (r *RecoverOptions) getMetaInfo(p string) (*meta.Data, error) {
	var buf []byte
	if _, err := fileops.Stat(p); err != nil {
		protocol := "http"
		if r.SSL {
		...
		u := fmt.Sprintf("%s://%s/getdata", protocol, r.Host)
		res, err := r.sendRequestToMeta(u, http.MethodGet, nil)
		if err != nil {
			return nil, err
		}
		defer res.Body.Close()
