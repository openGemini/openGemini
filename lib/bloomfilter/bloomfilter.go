...
// Minimum byte size required by each OneHitBloomFilter version. Add / Hit
// read 8 bytes starting at an offset derived from the hash value; if the
// buffer is smaller than maxOffset+8, a "slice bounds out of range" panic
// is raised. See https://github.com/openGemini/openGemini/issues/940.
//
//	V0/V1: offset = hash >> 49, max offset 32767, min size 32775 bytes
//	V2/V3: offset = hash >> 46, max offset 262143, min size 262151 bytes
const (
	minOneHitBloomFilterSizeV0 int64 = (1 << 15) + 7 // 32775
	minOneHitBloomFilterSizeV2 int64 = (1 << 18) + 7 // 262151
)

// MinOneHitBloomFilterSize returns the smallest byte size a buffer for the
// given OneHitBloomFilter version can have without risking a panic from
// Add / Hit on any valid hash value.
func MinOneHitBloomFilterSize(version uint32) int64 {
	if version <= 1 {
		return minOneHitBloomFilterSizeV0
	}
	return minOneHitBloomFilterSizeV2
}
...
func NewOneHitBloomFilter(bytes []byte, version uint32) Bloomfilter {
	if minSize := MinOneHitBloomFilterSize(version); int64(len(bytes)) < minSize {
		panic(fmt.Sprintf(
			"bloomfilter version %d requires at least %d bytes, got %d (see issue #940)",
			version, minSize, len(bytes),
		))
	}
	if version <= 1 {
