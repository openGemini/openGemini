...
func TestTranspiler_TranspileVectorSelector2ConditionExpr(t1 *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:    influxql.MustParseExpr("host::tag =~ /tele.*/"),
			wantErr: false,
		},
		{
			name: "2",
			...
			want:    influxql.MustParseExpr("host::tag =~ /tele.*/ AND cpu::tag = 'cpu0'"),
			wantErr: false,
		},
	}
...
func TestTranspiler_transpileVectorSelector(t1 *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:    influxql.MustParseStatement(`SELECT value AS value FROM cpu WHERE time >= '2023-01-06T04:00:00Z' AND time <= '2023-01-06T07:00:00Z' AND host::tag =~ /tele.*/ GROUP BY *`),
			wantErr: false,
		},
		{
			name: "2",
			...
			want:    influxql.MustParseStatement(`SELECT value AS value FROM cpu WHERE time >= '2023-01-08T02:00:00Z' AND time <= '2023-01-08T02:00:00Z' AND host::tag =~ /tele.*/ GROUP BY *`),
			wantErr: false,
		},
		{
			name: "3",
			...
			want:    influxql.MustParseStatement(`SHOW TAG VALUES ON prometheus FROM go_goroutines WITH KEY = job WHERE time >= '2023-01-08T02:00:00Z' AND time <= '2023-01-08T02:00:00Z' AND instance::tag =~ /192.168.*/`),
			wantErr: false,
		},
		{
			name: "4",
			...
			want:    influxql.MustParseStatement(`SHOW TAG VALUES ON prometheus FROM go_goroutines WITH KEY = "" WHERE time >= '2023-01-08T02:00:00Z' AND time <= '2023-01-08T02:00:00Z' AND instance::tag =~ /192.168.*/`),
			wantErr: false,
		},
		{
			name: "5",
			...
			want:    influxql.MustParseStatement(`SHOW TAG VALUES FROM go_goroutines WITH KEY = "" WHERE time >= '2023-01-08T02:00:00Z' AND time <= '2023-01-08T02:00:00Z' AND instance::tag =~ /192.168.*/`),
			wantErr: false,
		},
	}
	for _, tt := range tests {
...
func TestTranspiler_transpileRangeVectorSelector(t1 *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:    influxql.MustParseStatement(`SELECT value AS value FROM cpu WHERE time >= '2023-01-06T03:55:00Z' AND time <= '2023-01-06T07:00:00Z' AND host::tag =~ /tele.*/ GROUP BY *`),
			wantErr: false,
		},
		{
			name: "2",
