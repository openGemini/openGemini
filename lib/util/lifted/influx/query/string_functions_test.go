...
func TestElementAtFunc(t *testing.T) {
	e := query.GetMaterializeFunction("element_at", query.STRING)

	// Test GetRules method
	t.Run("TestGetRules", func(t *testing.T) {
		rules := e.GetRules("elementAt")
		if len(rules) != 3 {
			t.Errorf("Expected 3 rule, got %d", len(rules))
		}
		if rules[0].(*query.ArgNumberCheckRule).Min != 2 || rules[0].(*query.ArgNumberCheckRule).Max != 2 {
			t.Errorf(
				"Expected Min=2 and Max=2, got Min=%d and Max=%d",
				rules[0].(*query.ArgNumberCheckRule).Min,
				rules[0].(*query.ArgNumberCheckRule).Max)
		}
		if rules[1].(*query.TypeCheckRule).Index != 0 {
			t.Errorf(
				"Expected index=1, got index=%d", rules[1].(*query.TypeCheckRule).Index)
		}
		if rules[2].(*query.TypeCheckRule).Index != 1 {
			t.Errorf(
				"Expected index=2, got index=%d", rules[2].(*query.TypeCheckRule).Index)
		}
	})

	// Test CallTypeFunc method
	t.Run("TestCallTypeFunc", func(t *testing.T) {
		// Test case 1: Valid argument types
		dataType, err := e.CallTypeFunc("elementAt", []influxql.DataType{influxql.String, influxql.String})
		if err != nil {
			t.Errorf("Expected no error, but got: %v", err)
		}
		if dataType != influxql.String {
			t.Errorf("Expected String type, got %v", dataType)
		}
	})

	// Test CallFunc method
	t.Run("TestCallFunc", func(t *testing.T) {
		// Test case 1: Key exists
		input := "key1#$#value1|key2#$#value2"
		key := "key1"
		result, ok := e.CallFunc("elementAt", []interface{}{input, key})
		if !ok {
			t.Error("Expected value to be found, but got not found")
		}
		if result != "value1" {
		...
		result, ok = e.CallFunc("elementAt", []interface{}{input, key})
		if ok {
			t.Error("Expected value not to be found, but got found")
		}
		...
		result, ok = e.CallFunc("elementAt", []interface{}{input, key})
		if ok {
			t.Error("Expected value not to be found for empty input, but got found")
		}
		...
		result, ok = e.CallFunc("elementAt", []interface{}{input, key})
		if ok {
			t.Error("Expected value not to be found for empty key, but got found")
		}
		...
		result, ok = e.CallFunc("elementAt", []interface{}{input, key})
		if ok {
			t.Error("Expected value not to be found for invalid input format, but got found")
		}
	})
}
