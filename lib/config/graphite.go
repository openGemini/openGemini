...
https://github.com/influxdata/influxdb/blob/1.8/services/graphite/config.go
*/
...
func (c *GraphiteConfig) DefaultTags() models.Tags {
	if c == nil {
		fmt.Errorf("cannot get default tags because graphite config was nil")
	}
	...
	for _, t := range c.Tags {
		...
		if len(parts) != 2 {
			fmt.Errorf("invalid tag format: %s", t)
		}
		key := strings.TrimSpace(parts[0])
		value := strings.TrimSpace(parts[1])
		if key == "" {
			fmt.Errorf("invalid tag key: %s", t)
		}
		m[key] = value
	}
