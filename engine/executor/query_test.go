	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			dml: func(s *Storage) error {
				...
				builder := executor.NewChunkBuilder(rdt)
				chunk1 := builder.NewChunk("mst0")
				chunk1.AppendTime(1000000000, 2000000000, 3000000000, 4000000000, 5000000000)
				...
				chunk1.Column(2).AppendFloatValues(5, 4, 3, 2, 1)
				chunk1.Column(2).AppendManyNotNil(5)
				pts1 := influx.PointTags{influx.Tag{Key: "t", Value: "a"}}
				s.Write("db0.rp0.mst0", &pts1, chunk1)

				chunk2 := builder.NewChunk("mst0")
				chunk2.AppendTime(6000000000, 7000000000, 8000000000, 9000000000, 10000000000)
				chunk2.Column(0).AppendStringValues("a", "a", "a", "a", "a")
				chunk2.Column(0).AppendManyNotNil(5)
				chunk2.Column(1).AppendIntegerValues(6, 7, 8, 9, 10)
				chunk2.Column(1).AppendManyNotNil(5)
				chunk2.Column(2).AppendFloatValues(10, 9, 8, 7, 6)
