// Copyright 2023 Huawei Cloud Computing Technologies Co., Ltd.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package sparseindex_test

import (
	"encoding/binary"
	"hash/crc32"
	"math"
	"math/rand/v2"
	"os"
	"testing"

	"github.com/openGemini/openGemini/engine/index/sparseindex"
	"github.com/openGemini/openGemini/lib/bloomfilter"
	"github.com/openGemini/openGemini/lib/fragment"
	"github.com/openGemini/openGemini/lib/index"
	"github.com/openGemini/openGemini/lib/logstore"
	"github.com/openGemini/openGemini/lib/record"
	"github.com/openGemini/openGemini/lib/tokenizer"
	"github.com/openGemini/openGemini/lib/tracing"
	"github.com/openGemini/openGemini/lib/util/lifted/influx/influxql"
	"github.com/openGemini/openGemini/lib/util/lifted/influx/query"
	"github.com/openGemini/openGemini/lib/util/lifted/vm/protoparser/influx"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestSKIndexReader(t *testing.T) {
	dataFile := "00000001-0001-00000001.tssp"
	reader := sparseindex.NewSKIndexReader(2, 2, 0)
	option := &query.ProcessorOptions{Condition: &influxql.BinaryExpr{
		Op:  influxql.EQ,
		LHS: &influxql.VarRef{Val: "value", Type: influxql.Integer},
		RHS: &influxql.IntegerLiteral{Val: 2},
	}}
	mstInfo := &influxql.Measurement{IndexRelation: &influxql.IndexRelation{
		Oids:       []uint32{uint32(index.MinMax)},
		IndexNames: []string{index.MinMaxIndex},
		IndexList:  []*influxql.IndexList{{IList: []string{"value"}}},
	}}
	readers, err := reader.CreateSKFileReaders(option, mstInfo, true)
	assert.Equal(t, err, nil)

	readers[0] = &MockMinMaxIndexReader{}
	var frs = fragment.FragmentRanges{{Start: 0, End: 2}}
	for i := range readers {
		if err = readers[i].ReInit(dataFile); err != nil {
			t.Fatal(err)
		}
		frs, _, err = reader.Scan(readers[i], frs)
		assert.Equal(t, err, nil)
	}
	assert.Equal(t, len(frs), 1)
	assert.Equal(t, reader.Close(), nil)
}

type MockMinMaxIndexReader struct {
}

func (m MockMinMaxIndexReader) MayBeInFragment(fragId uint32) (bool, error) {
	if fragId == uint32(0) {
		return true, nil
	}
	return false, nil
}

func (m MockMinMaxIndexReader) GetFragmentRowCount(fragId uint32) (int64, error) {
	return 0, nil
}

func (m MockMinMaxIndexReader) ReInit(file interface{}) error {
	return nil
}

func (m MockMinMaxIndexReader) StartSpan(span *tracing.Span) {
}

func (m MockMinMaxIndexReader) Close() error {
	return nil
}

func TestSKBFIndexReader(t *testing.T) {
	reader := sparseindex.NewSKIndexReader(2, 2, 0)
	option := &query.ProcessorOptions{Condition: &influxql.BinaryExpr{
		Op:  influxql.EQ,
		LHS: &influxql.VarRef{Val: "value", Type: influxql.Integer},
		RHS: &influxql.IntegerLiteral{Val: 2},
	}}
	mstInfo := &influxql.Measurement{IndexRelation: &influxql.IndexRelation{
		Oids:       []uint32{uint32(index.BloomFilterFullText)},
		IndexNames: []string{index.BloomFilterFullTextIndex},
		IndexList:  []*influxql.IndexList{{IList: []string{"value"}}},
	}}
	r, err := reader.CreateSKFileReaders(option, mstInfo, true)
	assert.Equal(t, err, nil)
	err = r[0].Close()
	assert.Equal(t, err, nil)
}

func TestCreateSkipIndex(t *testing.T) {
	var colVal record.ColVal
	colVal.AppendStrings("hello", "world", "hello", "test")

	bloomFilterWriter := sparseindex.NewBloomFilterWriter("", "", "", "", tokenizer.CONTENT_SPLITTER)
	bytes := bloomFilterWriter.GenBloomFilterData(&colVal, []int{2, 2}, influx.Field_Type_String)

	segBfSize := int(logstore.GetConstant(2).FilterDataDiskSize)
	assert.Equal(t, 2*segBfSize, len(bytes))

	crc := crc32.Checksum(bytes[0:segBfSize-4], crc32.MakeTable(crc32.Castagnoli))
	assert.Equal(t, crc, binary.LittleEndian.Uint32(bytes[segBfSize-4:segBfSize]))

	bf := bloomfilter.NewOneHitBloomFilter(bytes[0:segBfSize-4], 3)
	assert.True(t, bf.Hit(tokenizer.Hash([]byte("hello"))))
	assert.True(t, bf.Hit(tokenizer.Hash([]byte("world"))))
	assert.False(t, bf.Hit(tokenizer.Hash([]byte("test"))))

	bf = bloomfilter.NewOneHitBloomFilter(bytes[segBfSize:2*segBfSize-4], 3)
	assert.True(t, bf.Hit(tokenizer.Hash([]byte("hello"))))
	assert.True(t, bf.Hit(tokenizer.Hash([]byte("test"))))
	assert.False(t, bf.Hit(tokenizer.Hash([]byte("world"))))
}

func TestSKIndexScanUsingMinMarksForSeek(t *testing.T) {
	reader := sparseindex.NewSKIndexReader(2, 2, 1)
	option := &query.ProcessorOptions{Condition: &influxql.BinaryExpr{
		Op:  influxql.EQ,
		LHS: &influxql.VarRef{Val: "value", Type: influxql.Integer},
		RHS: &influxql.IntegerLiteral{Val: 2},
	}}
	mstInfo := &influxql.Measurement{IndexRelation: &influxql.IndexRelation{
		Oids:       []uint32{uint32(index.MinMax)},
		IndexNames: []string{index.MinMaxIndex},
		IndexList:  []*influxql.IndexList{{IList: []string{"value"}}},
	}}
	readers, err := reader.CreateSKFileReaders(option, mstInfo, true)
	require.NoError(t, err)
	require.Equal(t, 1, len(readers))
	r := &MockMinMaxIndexReader{}

	var frs = fragment.FragmentRanges{{Start: 0, End: 4}}
	frs, _, err = reader.Scan(r, frs)
	require.NoError(t, err)
	require.Equal(t, 1, len(frs))
	require.Nil(t, reader.Close())
}

func TestSpatialIndexWriter(t *testing.T) {
	dir := t.TempDir()
	writer := sparseindex.NewSpatialWriter(dir, "0000", "", []string{"lat", "lon"})

	schema := record.Schemas{
		{Name: "lat", Type: influx.Field_Type_Float},
		{Name: "lon", Type: influx.Field_Type_Float},
	}

	var randFloat64 = func(maxV float64) float64 {
		return math.Floor(rand.Float64()*1e6*maxV) / 1e6
	}

	segCount := 5
	for i := range segCount {
		rec := &record.Record{}
		rec.ResetWithSchema(schema)
		for range 400 * i {
			rec.ColVals[0].AppendFloat(10 + randFloat64(0.05))
			rec.ColVals[1].AppendFloat(10 + randFloat64(0.05))
		}

		err := writer.CreateAttachIndex(rec, []int{0, 1}, nil)
		require.NoError(t, err)
		require.NoError(t, writer.FlushSegment())
	}

	require.NoError(t, writer.Flush())
	require.NoError(t, writer.Close())

	files := writer.Files()
	require.Equal(t, 1, len(files))

	buf, err := os.ReadFile(files[0])
	require.NoError(t, err)
	require.Greater(t, len(buf), 8)

	reader := sparseindex.NewSpatialReader(files[0], "")
	require.NoError(t, reader.Open())
	defer reader.Close()

	require.Equal(t, segCount, len(reader.SegmentOffsets()))
	for i := range segCount {
		segBuf, err := reader.ReadSegment(nil, i)
		require.NoError(t, err)
		require.Greater(t, len(segBuf), 0)
	}
}

func TestSpatialIndexWriterError(t *testing.T) {
	dir := t.TempDir()

	var writeInvalidRecord = func(rec *record.Record) {
		writer := sparseindex.NewSpatialWriter(dir, "0000", "", []string{"lat", "lon"})
		defer writer.Close()

		err := writer.CreateAttachIndex(rec, []int{0, 1}, nil)
		if err == nil {
			err = writer.FlushSegment()
		}
		require.Error(t, err)
	}

	schema := record.Schemas{
		{Name: "lat", Type: influx.Field_Type_Float},
		{Name: "lon", Type: influx.Field_Type_Float},
	}

	rec := &record.Record{}
	rec.ResetWithSchema(schema)
	rec.ColVals[0].AppendFloat(10 + rand.Float64())
	writeInvalidRecord(rec)

	schema[0].Type = influx.Field_Type_Int
	rec.ResetWithSchema(schema)
	rec.ColVals[0].AppendInteger(10)
	writeInvalidRecord(rec)

	schema[0].Type = influx.Field_Type_Float
	schema[0].Name = "invalid_name"
	rec.ResetWithSchema(schema)
	rec.ColVals[0].AppendFloat(10)
	rec.ColVals[1].AppendFloat(10)
	writeInvalidRecord(rec)
}
