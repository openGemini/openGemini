...
func NewFieldIndex(opts *Options) (*FieldIndex, error) {
	...
	fieldIndex.cache = LoadCache("TSID->Field", "tsid_field", fieldIndex.path, memory.Allowed()/32)
...
func (idx *FieldIndex) CreateIndexIfNotExists(primaryIndex PrimaryIndex, row *influx.Row, version uint16) (uint64, error) {
...
func (idx *FieldIndex) Search(primaryIndex PrimaryIndex, span *tracing.Span, name []byte, opt *query.ProcessorOptions) (GroupSeries, error) {
	version, ok := primaryIndex.GetVersion(name)
	if !ok {
		return nil, nil
	}
	vname := kbPool.Get()
	...
	// Firstly, get result from primary index, then we search based on result of primary index.
	groups, err := primaryIndex.SearchSeriesWithOpts(span, name, opt)
	if err != nil {
		return nil, err
	}

	sortedTagsSets := make(GroupSeries, 0, len(groups))
...
func FieldOpen(index interface{}) error {
	fieldIndex, ok := index.(*FieldIndex)
	if !ok {
		return fmt.Errorf("not a field index: %v", index)
	}
	return fieldIndex.Open()
}

func FieldInsert(index interface{}, primaryIndex PrimaryIndex, name []byte, row interface{}, version uint16) (uint64, error) {
	fieldIndex, ok := index.(*FieldIndex)
	if !ok {
		return 0, fmt.Errorf("not a field index: %v", index)
	}
	insertRow := row.(*influx.Row)
	return fieldIndex.CreateIndexIfNotExists(primaryIndex, insertRow, version)
}

func FieldScan(index interface{}, primaryIndex PrimaryIndex, span *tracing.Span, name []byte, opt *query.ProcessorOptions) (interface{}, error) {
	fieldIndex, ok := index.(*FieldIndex)
	if !ok {
		return nil, fmt.Errorf("not a field index: %v", index)
	}
	return fieldIndex.Search(primaryIndex, span, name, opt)
}

func FieldDelete(index interface{}, primaryIndex PrimaryIndex, name []byte, condition influxql.Expr, tr TimeRange) error {
	fieldIndex := index.(*FieldIndex)
	return fieldIndex.Delete(primaryIndex, name, condition, tr)
}

func FieldClose(index interface{}) error {
	fieldIndex := index.(*FieldIndex)
	return fieldIndex.Close()
}
