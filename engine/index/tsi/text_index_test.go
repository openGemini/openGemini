...
func getSearchExpr() influxql.Expr {
	...
	tExpr1 := &influxql.BinaryExpr{Op: influxql.MATCH_PHRASE, LHS: &influxql.VarRef{Val: "request"}, RHS: &influxql.StringLiteral{Val: "flashed_stage1.htm HTTP"}}
	...
	/*
	   condition:
	                         ...
	                         (OR)       AND
	                        /   \      /    \
	                     AND  tExpr0 tExpr1 OR
	                   /   \               /  \
	                dExpr0  dExpr1	  tExpr2 dExpr2
	*/
...
func getTagsetForTextSearchTest() *TagSetInfo {
	tagSet := &TagSetInfo{
		...
		RowFilters: make([][]clv.RowFilter, 0),
