...
func (enc *Time) Encoding(in []byte, out []byte) ([]byte, error) {
	times := record.Bytes2Uint64Slice(in)
	if len(times) < 3 {
...
func (enc *Time) constDeltaDecoding(out []byte) ([]byte, error) {
	...
	outPos := len(out)
	l := int(deltaCount+1) * record.Int64SizeBytes
	out = growBuffer(out, l)
	out = out[:l+outPos]

	outArr := record.Bytes2Int64Slice(out[outPos:])
...
func (enc *Time) simple8bDecoding(out []byte) ([]byte, error) {
	...
	times := record.Bytes2Uint64Slice(out[outPos:])
	times[0] = src[0]
	idx := 1
