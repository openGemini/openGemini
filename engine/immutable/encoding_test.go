...
func TestEncoding_FloatBlock(t *testing.T) {
	testFloat := func(outPrefix []byte, valueCount int) {
		...
		decOut := record.Float64Slice2byte(decodedValues)
		...
		if priLen > 0 {
			exp := record.Bytes2Float64Slice(outPrefix)
	...
	for _, row := range rows {
		testFloat(nil, row)
		testFloat(record.Float64Slice2byte(arr), row)
	}
}
...
func TestEncoding_FloatBlock_SimilarFloats(t *testing.T) {
	...
	decOut := record.Float64Slice2byte(decodedValues[:0])
...
func TestEncoding_IntBlock(t *testing.T) {
	testInt := func(prefix []byte, values []int64) {
		var err error
		var buf [1024]byte
		in := record.Int64Slice2byte(values)
		out := buf[:0]
		...
		decOut := record.Int64Slice2byte(decodedValues)
		...
		if preLen > 0 {
			get := record.Int64Slice2byte(outValues[:preLen/8])
	...
	for _, values := range inValues {
		testInt(nil, values)
		testInt(record.Int64Slice2byte(arr), values)
	}
}
...
func TestEncoding_IntBlock_Basic(t *testing.T) {
	...
	for _, count := range counts {
		testInt(nil, count)
		testInt(record.Int64Slice2byte(arr), count)
	}
}
...
func TestEncoding_IntBlock_randomdata(t *testing.T) {
	...
	intTest := func(prefix []byte) {
		_, err := safeRand.Read(record.Int64Slice2byte(values))
		if err != nil {
			t.Fatal(err)
		}
		...
		if preLen > 0 {
			get := record.Int64Slice2byte(outValue[:preLen/8])
			if !reflect.DeepEqual(get, prefix) {
				t.Fatalf("invlid uncompressed prefix data, exp:%v, get:%v", prefix, get)
			}
		}
	...
	intTest(record.Int64Slice2byte(arr))
}
...
func TestEncoding_BooleanBlock_Basic(t *testing.T) {
	boolTest := func(preData []byte, valueCount int) {
		...
		var err error
		var buf [64]byte
		in := record.BooleanSlice2byte(values)
		out := buf[:0]
		...
		decOut := record.BooleanSlice2byte(decodedValues[:])
	...
	for _, count := range counts {
		boolTest(nil, count)
		boolTest(record.BooleanSlice2byte(arr), count)
	}
}
...
func TestEncoding_Timestamp_Second(t *testing.T) {
	...
	tmTest := func(precision time.Duration, readTm bool, prefix []byte) {
		...
		var err error
		var buf [1024]byte
		in := record.Int64Slice2byte(times)
		...
		if preLen > 0 {
			preValue := record.Bytes2Int64Slice(prefix)
	...
	tmTest(time.Nanosecond, false, record.Int64Slice2byte(arr))

	tmTest(time.Second, true, nil)
	tmTest(time.Second, true, record.Int64Slice2byte(arr))
	tmTest(time.Millisecond, true, nil)
	tmTest(time.Millisecond, true, record.Int64Slice2byte(arr))
	tmTest(time.Nanosecond, true, nil)
	tmTest(time.Nanosecond, true, record.Int64Slice2byte(arr))
}

func TestEncodeIncompressibleInteger(t *testing.T) {
	origArr := make([]int64, 500)
	values := make([]int64, 500)
	buf := record.Int64Slice2byte(origArr)
