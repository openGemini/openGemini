...
func TestTaskExport2TSSPFile(t *testing.T) {
	convey.Convey("test export tsspFile", t, func() {
		...
		p3 := gomonkey.ApplyMethod(fakeIterator, "Close", func() {})
		defer p3.Reset()

		task := &ParquetTask{}

		convey.Convey("get series id failed", func() {
			err := task.exportFromTSSPFile(&f, &parquet.Writer{})
		...
		convey.Convey("write ok", func() {
			...
			p6 := gomonkey.ApplyMethod(fakeWriter, "ForceFlush", func(_ *parquet.Writer) error {
				return nil
			})
			defer p6.Reset()

			err := task.exportFromTSSPFile(&f, fakeWriter)
...
func TestParquetTaskProcess(t *testing.T) {
	convey.Convey("test task process", t, func() {
		...
		convey.Convey("test export2TSSPFile failed", func() {
			...
			p1 := gomonkey.ApplyPrivateMethod(&task, "exportFromTSSPFile", func(_ *ParquetTask, _ TSSPFile, _ *parquet.Writer) error {
...
func TestGetDateDirWithTimeZone(t *testing.T) {
	convey.Convey("test get date dir with time zone", t, func() {
		defer func() {
			err := config.UpdateTimeZoneLoc("UTC")
			convey.So(err, convey.ShouldBeNil)
		}()
		parquetInfo := parquetFileInfo{shardInfo: "5_1735747201000000000_1735833601000000000"}
		err := parquetInfo.getDateDir()
		convey.So(parquetInfo.dt, convey.ShouldEqual, "2025-01-01")

		err = config.UpdateTimeZoneLoc("Asia/Shanghai")
		convey.So(err, convey.ShouldBeNil)
		parquetInfo.dt = ""
		err = parquetInfo.getDateDir()
		convey.So(err, convey.ShouldBeNil)
		convey.So(parquetInfo.dt, convey.ShouldEqual, "2025-01-02")

		convey.Convey("test set wrong timeZone", func() {
			err = config.UpdateTimeZoneLoc("Asia/BJ")
			convey.So(err, convey.ShouldNotBeNil)
...
func TestSkipDisableMsts(t *testing.T) {
	convey.Convey("test skip disable msts", t, func() {
		paths := []string{"/tsdb/instanceId/data/db/2/autogen/255_1723420800000000000_1723507200000000000_231/tssp/mst01_0000/0000086d-0002-00000000.tssp",
			"/tsdb/instanceId/data/db01/3/autogen/260_1723420800000000000_1723507200000000000_236/tssp/mst_0000/00000061-0002-00000000.tssp",
			"/tsdb/instanceId/data/db/0/autogen/253_1723420800000000000_1723507200000000000_229/tssp/mst_0000/000007a2-0002-00000000.tssp"}
		plan := TSSP2ParquetPlan{Files: paths}
		task := ParquetTask{plan: plan}

		conf := config.GetStoreConfig().ParquetTask
		conf.SetEnableMst([]string{"db.autogen.mst"})
		defer conf.SetEnableMst([]string{})

		ans, err := task.prepare()
		convey.So(err, convey.ShouldBeNil)
		convey.So(len(ans), convey.ShouldEqual, 1)
	})
}

func TestSkipInternal(t *testing.T) {
	convey.Convey("test parquet task skip internal", t, func() {
		paths := []string{"/tsdb/instanceId/data/_internal/dbpt/rp/shardId_1714867200000000000_1715040000000000000_872/tssp/mst/00000001-0000-00000000.tssp",
			"/tsdb/instanceId/data/_internal/dbpt/rp/shardId_1714867200000000000_1715040000000000000_872/tssp/mst/out-of-order/00000001-0000-00000000.tssp",
			"/tsdb/instanceId/data/test/dbpt/rp/shardId_1714867200000000000_1715040000000000000_872/tssp/mst/00000001-0000-00000000.tssp",
			"/tsdb/instanceId/data/_internal/dbpt/rp/shardId_1714867200000000000_1715040000000000000_872/tssp/mst/out-of-order/00000002-0000-00000000.tssp"}
		plan := TSSP2ParquetPlan{Files: paths}
		task := ParquetTask{plan: plan}

		p1 := gomonkey.ApplyFunc(fileops.MkdirAll, func(_ string, _ os.FileMode, _ ...fileops.FSOption) error {
			return nil
		})
		defer p1.Reset()

		mapping, err := task.prepare()
		convey.So(len(mapping), convey.ShouldEqual, 1)
		convey.So(err, convey.ShouldBeNil)
	})
}
