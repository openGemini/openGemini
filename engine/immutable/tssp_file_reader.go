...
type TSSPFileReader interface {
	Open() error
	Close() error
	ReadAt(cm *ChunkMeta, segment int, dst *record.Record, ctx *ReadContext) (*record.Record, error)
	Ref()
	Unref()
	...
	ContainsValue(id uint64, tm record.TimeRange) (bool, error)
	ContainsTime(tm record.TimeRange) (bool, error)
...
func (r *tsspFileReader) ReadAt(cm *ChunkMeta, segment int, dst *record.Record, decs *ReadContext) (*record.Record, error) {
...
func (r *tsspFileReader) readSegmentRecord(cm *ChunkMeta, segment int, dst *record.Record, decs *ReadContext) (*record.Record, error) {
	...
	for i := range schema[:len(schema)-1] {
		...
		if len(chunkData) > 0 {
			...
			if err != nil {
				log.Error("read column data fail", zap.String("file", r.Path()), zap.String("col", cMeta.name), zap.Error(err))
				return nil, err
			}
		}
		...
		if err != nil {
			err = errReadFail(r.Path(), ref.Name, err)
...
func (r *tsspFileReader) decodeTimeColumn(cm *ChunkMeta, segment int, chunkData []byte,
	...
	if len(chunkData) > 0 {
		...
		if err != nil {
			log.Error("read time column fail", zap.String("file", r.Path()), zap.Error(err))
			return err
		}
	}
	...
	if err != nil {
		err = errReadFail(r.Path(), "time", err)
...
func (r *tsspFileReader) ReadMetaBlock(metaIdx int, id uint64, offset int64, size uint32, count uint32,
	...
	if offset < mOff || end > mOff+mSize {
		...
		log.Error("read chunk meta fail", zap.String("file", r.Path()), zap.Error(err))
		return nil, err
	}
	...
	if readCacheEn {
		...
		rb, err = r.ReadData(offset, size, dst)
	}
	if err != nil {
...
func (r *tsspFileReader) ReadDataBlock(offset int64, size uint32, dst *[]byte) (rb []byte, err error) {
	...
	if err != nil {
		log.Error("read file failed", zap.String("file", r.Path()), zap.Error(err))
		return nil, err
	}
...
func (r *tsspFileReader) GetTSSPFileBytes(offset int64, size uint32, buf *[]byte) ([]byte, error) {
	...
	cacheKey := cacheIns.CreatCacheKey(r.Path(), offset)
	var b []byte
	...
	b, err = r.ReadData(offset, size, buf)
	if err != nil {
...
func (r *tsspFileReader) ReadData(offset int64, size uint32, dst *[]byte) ([]byte, error) {
...
func (r *tsspFileReader) FileStat() *Trailer {
	return &r.trailer
}
...
func (r *tsspFileReader) ContainsValue(id uint64, tm record.TimeRange) (bool, error) {
...
func (r *tsspFileReader) ContainsTime(tm record.TimeRange) (bool, error) {
	return tm.Overlaps(r.trailer.minTime, r.trailer.maxTime), nil
}

func (r *tsspFileReader) Contains(id uint64) (bool, error) {
	tr := record.TimeRange{Min: influxql.MinTime, Max: influxql.MaxTime}
	return r.ContainsValue(id, tr)
}
...
func (r *tsspFileReader) Path() string {
	return r.r.Name()
}
...
var (
	_ TSSPFileReader = (*tsspFileReader)(nil)
)
