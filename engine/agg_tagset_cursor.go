...
func (s *fileLoopCursor) FilterRecInMemTable(re *record.Record, seriesKey *seriesInfo) (*record.Record, *comm.FileInfo) {
	...
	if re != nil {
		...
		filterOpts = immutable.NewFilterOpts(s.tagSetInfo.Filters[index], s.ctx.m, s.ctx.filterFieldsIdx, s.ctx.filterTags, &seriesKey.tags, s.tagSetInfo.RowFilters[index])
