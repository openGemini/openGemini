...
func NewRecordSchema(querySchema *executor.QuerySchema, auxTags []string, schema record.Schemas, filterConditions []*influxql.VarRef, engineType config.EngineType) ([]string, record.Schemas) {
	fieldMap := make(dictmap.DictMap[string, influxql.DataType])
...
func genSchemaByRef(fieldMap dictmap.DictMap[string, influxql.DataType], key string, ref *influxql.VarRef, schema *record.Schemas, auxTags *[]string, engineType config.EngineType, queryRef bool) {
	switch ref.Type {
	case influxql.Integer, influxql.String, influxql.Boolean, influxql.Float:
		{
			fieldType := ref.Type
			_, ok := fieldMap.Get(key)
			if !ok {
