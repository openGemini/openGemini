...
(
GOOS=linux  go list ./... | grep -vE "tests|lifted" | xargs staticcheck -go=1.18 -tests=false -f binary
GOOS=darwin go list ./... | grep -vE "tests|lifted" | xargs staticcheck -go=1.18 -tests=false -f binary
