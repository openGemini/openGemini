...
(
GOOS=linux  go list ./... | grep -vE "tests|open_src|lifted" | xargs staticcheck -go=1.18 -tests=false -f binary
GOOS=darwin go list ./... | grep -vE "tests|open_src|lifted" | xargs staticcheck -go=1.18 -tests=false -f binary
