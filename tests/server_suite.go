...
func init() {
	...
	tests["measurement_commands"] = Test{
		queries: []*Query{
			...
			&Query{
				name:    "retry create measurement testindex",
				params:  url.Values{"db": []string{"db0"}},
				command: `CREATE MEASUREMENT testindex(tag1 string TAG, tag2 string TAG, field1 string FIELD, field2 string FIELD, field3 string FIELD, INDEX index1 field2 TYPE TEXT, INDEX index2 field3 TYPE keyword) `,
				exp:     `{"results":[{"statement_id":0}]}`,
				once:    true,
			},
			&Query{
				name:    "retry create measurement shardkey testindex2",
				params:  url.Values{"db": []string{"db0"}},
				command: `CREATE MEASUREMENT testindex2(tag1 string TAG, tag2 string TAG, field1 string FIELD, field2 string FIELD, field3 string FIELD, INDEX index1 field2 TYPE TEXT, INDEX index2 field3 TYPE keyword) WITH SHARDKEY hostname`,
				exp:     `{"results":[{"statement_id":0}]}`,
				once:    true,
			},
			&Query{
				...
				params:  url.Values{"db": []string{"db0"}},
				command: `SHOW MEASUREMENTS`,
				exp:     `{"results":[{"statement_id":0,"series":[{"name":"measurements","columns":["name"],"values":[["cpu"],["testindex"],["testindex2"]]}]}]}`,
				once:    true,
			},
			&Query{
				name:    "drop measurement testindex",
				params:  url.Values{"db": []string{"db0"}},
				command: `DROP MEASUREMENT testindex`,
				exp:     `{"results":[{"statement_id":0}]}`,
				once:    true,
			},
			&Query{
				name:    "drop measurement testindex2",
				params:  url.Values{"db": []string{"db0"}},
				command: `DROP MEASUREMENT testindex2`,
				exp:     `{"results":[{"statement_id":0}]}`,
				once:    true,
			},
			&Query{
