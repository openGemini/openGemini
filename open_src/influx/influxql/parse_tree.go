...
This code is originally from: https://github.com/influxdata/influxql/blob/v1.1.0/parse_tree.go

2022.01.23 Add Group function for ParseTree.
...
func init() {
	...
	Language.Group(CREATE).With(func(create *ParseTree) {
		...
		create.Handle(MEASUREMENT, func(p *Parser) (Statement, error) {
			return p.parseCreateMeasurementStatement()
		})
	})
	...
	Language.Group(ALTER).With(func(alter *ParseTree) {
		...
		alter.Handle(MEASUREMENT, func(p *Parser) (Statement, error) {
			return p.parseAlterShardKeyStatement()
		})
	})
	...
	Language.Group(PREPARE).With(func(prepare *ParseTree) {
		prepare.Handle(SNAPSHOT, func(p *Parser) (Statement, error) {
			return p.parsePrepareSnapshotStatement()
		})
	})
	Language.Group(END).With(func(prepare *ParseTree) {
		prepare.Handle(SNAPSHOT, func(p *Parser) (Statement, error) {
			return p.parseEndPrepareSnapshotStatement()
		})
	})
	Language.Group(GET).With(func(prepare *ParseTree) {
		prepare.Handle(RUNTIMEINFO, func(p *Parser) (Statement, error) {
			return p.parseGetRuntimeInfoStatement()
		})
	})
}
