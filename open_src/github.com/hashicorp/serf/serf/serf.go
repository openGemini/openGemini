...
type Serf struct {
	...
	coordClient    *coordinate2.Client
	coordCache     map[string]*coordinate2.Coordinate
...
func Create(conf *Config, clock uint64, preNodes []*PreviousNode) (*Serf, error) {
	conf.Init()
	...
	serf := &Serf{
		...
		state:         SerfAlive,
		Clock:         LamportClock{counter: clock},
	}
	...
	if !conf.DisableCoordinates {
		serf.coordClient, err = coordinate2.NewClient(coordinate2.DefaultConfig())
		if err != nil {
	...
	if !conf.DisableCoordinates {
		serf.coordCache = make(map[string]*coordinate2.Coordinate)
...
func (s *Serf) GetCoordinate() (*coordinate2.Coordinate, error) {
	if !s.config.DisableCoordinates {
...
func (s *Serf) GetCachedCoordinate(name string) (coord *coordinate2.Coordinate, ok bool) {
	if !s.config.DisableCoordinates {
