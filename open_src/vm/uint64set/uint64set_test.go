...
var sequenceID = uint64(time.Now().Unix())

func GenerateUUID() uint64 {
	clock := uint64(1 + rand.Intn(3))
	b := kbPool.Get()
	// first three bytes is big endian of logicClock
	b.B = append(b.B, byte(clock>>16))
	b.B = append(b.B, byte(clock>>8))
	b.B = append(b.B, byte(clock))
...
func TestNormalSetIterator(t *testing.T) {
	f := func(itemsCount int, sorted bool, t *testing.T) {
		var s Set
		ids := make([]uint64, 0, itemsCount)
		count := 0
		for i := 0; i < itemsCount; i++ {
		...
		itr := s.Clone().Iterator()
		for itr.HasNext() {
			next := itr.Next()
			if next != ids[count] {
				t.Fatalf("next [%d] not equal expected value [%d]\n", next, ids[count])
			}
			count++
		}
		assert.Equal(t, int(itemsCount), count, "count not equal expected value ", itemsCount)
	}

	for _, itemsCount := range []int{1e3, 1e4, 1e5, 1e6} {
		t.Run(fmt.Sprintf("items_sorted_%d", itemsCount), func(t *testing.T) {
			f(itemsCount, true, t)
		})

		t.Run(fmt.Sprintf("items_unsorted_%d", itemsCount), func(t *testing.T) {
			f(itemsCount, false, t)
		})
	}
}

func TestSpecialSetIterator(t *testing.T) {
	// exists nil bucket16
	t.Run("exists_nil_bucket16", func(t *testing.T) {
		var s1, s2 Set
		s1.AddMulti([]uint64{4399702310892, 5399702310892})
		s2.Add(5399702310892)
		s1.Intersect(&s2)

		itr := s1.Iterator()
		for itr.HasNext() {
			next := itr.Next()
			assert.Equal(t, uint64(5399702310892), next)
		}
		assert.Equal(t, 1, itr.count)
	})

	// exists empty but not nil bucket16
	t.Run("exists_empty_bucket16", func(t *testing.T) {
		var s Set
		s.AddMulti([]uint64{4399702310892, 5399702310892})
		s.Del(4399702310892)
		itr := s.Iterator()
		for itr.HasNext() {
			next := itr.Next()
