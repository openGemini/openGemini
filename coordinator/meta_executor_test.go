...
func TestMetaExecutor(t *testing.T) {
	...
	err := me.EachDBNodes("db_not_exists", func(nodeID uint64, pts []uint32) {})
	assert.True(t, errno.Equal(err, errno.DatabaseNotFound))

	err = me.EachDBNodes("dbpt_not_exists", func(nodeID uint64, pts []uint32) {})
	assert.True(t, errno.Equal(err, errno.DatabaseNotFound))

	err = me.EachDBNodes("db0", func(nodeID uint64, pts []uint32) {})
